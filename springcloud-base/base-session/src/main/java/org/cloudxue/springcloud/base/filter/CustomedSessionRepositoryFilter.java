...
/**
 * @ClassName CustomedSessionRepositoryFilter
 * @Description 将request和response进行包装，将HttpSession替换成RedisSession
 * @Author xuexiao
...
public class CustomedSessionRepositoryFilter<S extends Session> extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        // 包装原始 HttpServletRequest 至 SessionRepositoryRequestWrapper
        SessionRepositoryRequestWrapper wrappedRequest =
                new SessionRepositoryRequestWrapper(request, response, this.servletContext);

        // 包装原始 HttpServletResponse 响应至 SessionRepositoryResponseWrapper
        ...
        private S getRequestedSession() {
            if (!this.requestedSessionCached) {
                List<String> sessionIds =
                        CustomedSessionRepositoryFilter.this.httpSessionIdResolver.resolveSessionIds(this);
                log.info("======当前sessionIds: " + sessionIds.toString());

                for (String sessionId : sessionIds) {
                    if (this.requestedSessionId == null) {
                        this.requestedSessionId = sessionId;
                    }
                    S session = CustomedSessionRepositoryFilter.this.sessionRepository.findById(sessionId);
                    if (session != null) {
